...
@section('title', 'Cobro ' . $order->order_number . ' - Facturación')
...
<div class="module-page">
    <section class="module-hero">
        <div>
            ...
            <p>Registra el pago de la mesa y decide si el cliente necesita ticket normal o factura electrónica con CUFE.</p>
        </div>
        <div class="summary-group">
            <span class="summary-chip">{{ $restaurantTable?->name ?? 'Mesa no disponible' }}</span>
            <span class="summary-chip">{{ $restaurantTable?->area ?: 'Salón principal' }}</span>
                ...
                <div class="card-body">
                    ...
                    @else
                        <div class="meta-box mb-3">
                            ...
                            <div class="seat-note">Código {{ $activeBox->code }}</div>
                        ...
                        <form method="POST" action="{{ route('billing.checkout.store', $order) }}" id="billingCheckoutForm" accept-charset="UTF-8">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-help mt-1" id="amountReceivedHelp">Ingresa el valor recibido y verás de inmediato el cambio a devolver.</div>
                            </div>

                            <div class="mb-3">
                                <label class="form-label" for="reference">Referencia</label>
                                <input type="text" class="form-control" id="reference" name="reference" value="{{ old('reference') }}" placeholder="Últimos dígitos, comprobante o nota del pago">
                            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const billingReadiness = @json($billingReadiness);

    if (!methodSelect || !tipInput || !amountReceivedInput || !amountDueLabel || !changeLabel || !checkoutForm || !documentTypeSelect) {
        return;
    }
    ...
    const syncDocumentHelp = () => {
        const isElectronic = documentTypeSelect.value === 'electronic';

        documentTypeHelp.textContent = isElectronic
            ? (billingReadiness.ready
                ? 'Se intentará emitir factura electrónica y el documento mostrará el CUFE cuando Factus responda.'
                : 'La factura electrónica requiere un cliente completo y la configuración activa de Factus.')
            : 'El ticket se imprime de inmediato y no requiere CUFE.';
    };
    ...
    const syncAmounts = () => {
        ...
        if (requiresExactAmount) {
            ...
            amountReceivedHelp.textContent = method
                ? 'Ingresa el valor recibido y verás de inmediato el cambio a devolver.'
                : 'Ingresa el valor recibido y el sistema te mostrará el cambio al instante.';
        }
    ...
    documentTypeSelect.addEventListener('change', syncDocumentHelp);
            ...
            if (window.Swal) {
                await Swal.fire({
                    ...
                    text: data?.cufe
                        ...
                        : (data?.message || 'El documento se está preparando para impresión.'),
