    ...
    public function index(Request $request): View|Response
    {
        ...
        return view('reports.index', [
            ...
            'summary' => [
                ...
                'gross_sales' => money_value((float) $summaryRows->sum('subtotal')),
                'discounts' => money_value((float) $summaryRows->sum('discount_amount')),
                'taxes' => money_value((float) $summaryRows->sum('tax_amount')),
                'net_revenue' => money_value((float) $summaryRows->sum('total')),
                'average_ticket' => money_value($summaryRows->count() > 0 ? (float) $summaryRows->avg('total') : 0),
                'cash_income' => money_value((float) $cashMovements->where('amount', '>', 0)->sum('amount')),
                'cash_expense' => money_value(abs((float) $cashMovements->where('amount', '<', 0)->sum('amount'))),
        ...
        return response()->streamDownload(function () use ($sales): void {
            ...
            foreach ($sales as $sale) {
                fputcsv($handle, [
                    ...
                    money_input($sale->subtotal),
                    money_input($sale->discount_amount),
                    money_input($sale->tax_amount),
                    money_input($sale->total),
